...
export function ghostMannequinPrompt(spec: {
  ...
  return [
    ...
    `fabric folds.`,
    ``,
    `Show ONLY this one garment. Remove the person entirely and remove every`,
    `other item they are wearing — no head, face, hair, hands, arms, legs, feet,`,
    `no trousers, palazzo, shalwar or under-layer, no shoes, no jewellery, no`,
    `mannequin or stand. Below the hem there must be nothing but empty backdrop.`,
    ...(shape ? ["", shape] : []),
    ``,
